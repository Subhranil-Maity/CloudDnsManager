...
- **API Token Verification** — Enter your Cloudflare API token and the app verifies it immediately
- **Zone Browsing** — View all zones (domains) associated with your account
...
1. **State**: An immutable data class holding everything needed to render the UI
2. **Intent**: A sealed class representing every possible user action
3. **ViewModel**: Processes intents, performs side effects, and emits new state

This means the Compose UI is a pure function of state — easy to test, reason about, and debug. No side effects in composables, no hidden state.
